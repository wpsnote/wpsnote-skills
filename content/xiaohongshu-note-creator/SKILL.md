...
**这是一个硬阻断检查点。AI 必须在对话中逐条打印以下 checklist，全部勾选后才能继续。缺少任何一条，视为流程非法，必须回退。**

#### 执行步骤（必须按顺序完成，不可跳过、不可合并）
...
扫描后必须在对话中打印：
```
[读图检查] 扫描完成，共发现 {N} 个图片 block：
- block_id: {id1}（位置：第{X}段附近）
- block_id: {id2}（位置：第{X}段附近）
...
```
若 N=0，打印「[读图检查] 原文无图片」，跳至步骤④。

**② 逐张读取 base64（每张都要执行，不可跳过）**
```bash
wpsnote-cli read-image --note_id {note_id} --block_id {block_id} --json
```
MCP 降级：`read_image({ note_id, block_id })`

**③ 逐张进行视觉描述（必须在对话中打印，不可省略）**

每张图读取成功后立刻打印：
```
[图片 {N}] block_id: {id}
  画面内容：{图里有什么人/物/场景/元素，具体描述}
  风格色调：{插画/摄影/扁平/线条 + 颜色描述}
  建议用于：P{页码}（原因：{与该页内容的关联}）
  垫图可用：{是（openrouter/gemini）/ 否（ark/dashscope跳过）}
```

**④ 打印最终 checklist（全部勾选才能继续）**
```
[读图检查点完成]
✅ 已扫描图片 block：共 {N} 张
✅ 已逐张读取 base64
...
> 🚨 **如果没有在对话中打印上述 checklist，不得输出任何文案内容。用户看到 AI 直接开始写内容，有权打断并要求重新从此检查点开始。**
