...
### 0. 读图强制检查点（🚨 最高优先级，不完成禁止进入任何后续步骤）

**这是硬阻断检查点。AI 必须先调用工具，再把工具返回的真实数据打印出来。打印内容必须包含工具调用的实际返回值（真实 block_id、真实图片描述），不允许出现任何占位符（如 `{id}`、`{block_id}`、`...`）。**

> 🚨 **用户识别造假的方法**：如果打印的 block_id 是占位符格式，说明 AI 没有真正调用工具。用户可以直接打断并要求重做。

#### 执行步骤（必须按顺序，不可跳过、不可合并、不可提前输出文案）

**① 调用 outline，打印真实扫描结果**
```bash
wpsnote-cli outline --note_id {note_id} --json
```
调用完成后，打印工具实际返回的图片 block 信息：
```
[读图检查①] outline 调用完成，共发现 N 个图片 block：
- block_id: abc123xyz（这是工具返回的真实ID，不是占位符）
- block_id: def456uvw
```
若无图片：打印「[读图检查①] 原文无图片，跳至④」

**② 逐张调用 read-image，打印每张图的真实 base64 前缀（证明已读取）**
```bash
wpsnote-cli read-image --note_id {note_id} --block_id {真实block_id} --json
```
MCP 降级：`read_image({ note_id, block_id })`

每张读取后打印：
```
[读图检查②] block_id abc123xyz 读取成功，base64 前20字符：/9j/4AAQSkZJRgABAQAA...
```
（base64 前缀是工具返回的真实数据，用于证明调用成功）

**③ 逐张视觉描述（基于②的 base64 内容，不可凭空编写）**

每张描述必须打印：
```
[读图检查③] block_id abc123xyz 视觉描述：
  画面内容：（具体描述图里有什么，如"年轻人坐在桌前，屏幕上有Notion图标"）
  风格色调：（如"扁平插画风，蓝白配色"）
  建议用于：P2（与该页主题"笔记软件周期律"相关）
```

**④ 打印完成 checklist（block_id 必须是真实值）**
```
[读图检查点完成]
✅ outline 已调用，发现图片 block：abc123xyz, def456uvw
✅ read-image 已逐张调用，base64 已获取
✅ 视觉描述已完成（见上方③）
✅ 配图方案已确认
...
> 🚨 **如果上方任一步骤缺失，或 block_id 是占位符，视为未执行读图。后续输出的文案方案无效，必须从①重新开始。**
