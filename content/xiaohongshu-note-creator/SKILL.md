...
### 0. 读图强制检查点（必须最先执行，违反则整个流程无效）

**在写任何一页文案之前，必须完成以下步骤，否则视为违规，必须回退重做：**

**步骤一：扫描图片 block**
```bash
wpsnote-cli outline --note_id {note_id} --json
# 找出所有 type="image" 的 block，记录其 block_id
```

**步骤二：逐张读取并描述（每张图都要做，不可跳过）**
```bash
wpsnote-cli read-image --note_id {note_id} --block_id {block_id} --json
# 取 data.image_base64，立刻用视觉理解描述：
# - 画面内容：图里有什么人/物/场景/元素
# - 风格色调：插画/摄影/扁平/线条/色调
# - 适合用于：哪页（根据图片内容判断）
```
...
**步骤三：汇报读图结果**
在对话中说：「原文共 {N} 张图片，已全部解析：图1[...] 图2[...]，将用于 P{页码} 垫图。」

**无图片时**：说「原文无图片，将使用纯文生图」，然后继续。

> ⚠️ 如果跳过读图直接输出方案，用户有权要求重做。AI 不得以"文字转化完成"为由略过读图步骤。
