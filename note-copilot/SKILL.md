---
name: note-copilot
description: 笔记协作助手：帮用户整理当前 WPS 笔记，识别并处理笔记中的 *** 和 /// 援助标记，同时在发现明显逻辑错误时以着色方式提醒。当用户说「帮我整理笔记」「整理一下」「帮我看看笔记」「处理一下标记」「定期帮我整理」，或提到笔记中有 *** 或 /// 需要处理时使用此 skill。
metadata:
  author: Yicheng Xu
  version: "1.0.0"
...
陪伴用户整理笔记的 AI 助手。扫描当前笔记，处理用户留下的 `***`（补全）和 `///`（引用）标记，并在发现明显逻辑错误时轻提醒。
...
定期模式（用户明确说「定期帮我整理」时才启动）：
  执行一次整理 → sleep 300秒 → 重复
  用户说「停止」时退出
```

## 首次使用引导

**判断方式**：本次对话中是否已经输出过引导说明。若未输出过，则在开始整理前先输出：

```
笔记协作助手已就绪 ✓

我可以帮你整理当前笔记，处理你留下的标记：
...
仅当用户明确说「定期帮我整理」「每 5 分钟帮我整理」时启动：

```
启动时：
  告知用户：「已开启定期整理，每 5 分钟自动扫描当前笔记。说「停止」可随时退出。」
