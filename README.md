...
| `search/` | 搜索与阅读：新闻解读、智能深搜、文献阅读 |
| `capture/` | 信息捕获与记录：音频转写、文档导入、编码技术文档 |
...
| `tag-organize` | 清理混乱标签、重构标签体系、优化分类结构 |
...
想要你的 Skill 覆盖海量用户吗？WPS 笔记的庞大用户基数，就是你 AI 创意的最佳放大器。加入 AI Skill 共创社区，让你的本地 Skill 直接跃入无数人的业务工作流。
...
- **带着代码来（Pull Request）**：带上你写好的 Skill 发起 PR，并[扫码加入群聊](https://365.kdocs.cn/l/cau3Z9vsAUQt)。我们会协助排查场景冲突、接入底层接口，推动它以最快速度合并至官方仓库
- **带着脑洞来（Issue）**：想到一个高价值的空白场景？抛出你的 Issue，并[扫码加入群聊](https://365.kdocs.cn/l/cau3Z9vsAUQt)。社区里永远有手痒的小伙伴，他们会接力把你的好点子变成现实

你的每一个灵感，都可能成为 WPS 笔记未来的原生 AI 能力。

---

## 如何贡献 Skill

我们欢迎社区共建。如果你有一个想法，想把它变成所有人都能用的 Skill，请按照以下流程操作：
...
**方式二：Issue（想法阶段）**
- 如果你只有一个想法，还没动手实现，可以先开一个 Issue
- 描述你想要的 Skill 的功能和适用场景
- 社区成员可以一起讨论和接力实现
...
- **海量的真实调用**：一段精妙的 Prompt 或触发逻辑，能迅速成为无数人每天高频唤醒的效率利器
- **定义官方规范**：优秀的底层能力和防冲突设计，将沉淀为官方仓库的底层标准，指引后来的伙伴
- **永久的极客署名**：成功合并进主分支的优质 Skill，仓库会保留每一次 Commit，你的 ID 将永久刻在核心贡献者名单里

扫码加入群聊，带上你的代码或脑洞，我们协助排查场景冲突、接入底层接口，推动它以最快速度合并至官方仓库：
