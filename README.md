...
图形化配置，开箱即用。20+ 工具覆盖笔记完整生命周期——创建、编辑、搜索、归档、图片插入、音频卡片、标签管理、跨笔记本检索、分段内容返回。
...
literature-reader/               # 场景：学术文献阅读与结构化概要生成
  SKILL.md
  references/
    pdf-extract.md
    summary-template.md
...
学术文献阅读与结构化概要生成，支持 PDF 论文的元信息提取、方法论梳理、实验结果分析，以及多篇文献横向对比。优先使用 Read 工具直接读取 PDF，备选方案基于 pdfplumber 提取。

- [Skill 定义](./literature-reader/SKILL.md)
...
`news-to-note`、`tag-organize`、`note-copilot`、`live-transcript-summary`、`coding-assistant`、`literature-reader` 等目录聚焦具体任务流程，复用 `wps-note` 的 MCP 或 CLI 能力完成业务目标。
